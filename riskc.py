...
def main(stdscr):
    ...
    stdscr.addstr(2, 0, "  Offense")
    stdscr.addstr(3, 0, dice.join(dice.str(1), dice.str(1), dice.str(1)))
    stdscr.addstr(8, 0, "  Defense")
    stdscr.addstr(9, 0, dice.join(dice.str(1), dice.str(1)))

    ch = ord('1')
    while ch != ord('q'):
        offense = get_int(stdscr, 1, "Number of offensive troops")
        defense = get_int(stdscr, 1, "Number of defensive troops")

        stdscr.addstr(1, 0, "Hit ' ' to automatically finish battles, 'q' to quit, any other for manual play")

        battle = Battle(Army(offense), Army(defense))
        stdscr.addstr(14, 0, str(battle))
        ...
        if auto == False:
            stdscr.addstr(1, 0, "Hit ' ' to continue battle, 'q' to quit, any other to stop                     ")
        else:
            stdscr.addstr(1, 0, "                                                                               ")
        ...
        while battle.action() and ch != ord('q'):
            ...
            stdscr.addstr(3, 0, dice.join(*[dice.str(r) for r in o_roll]))
            stdscr.addstr(9, 0, "{:20}\n".format(" ") * 5)
            stdscr.addstr(9, 0, dice.join(*[dice.str(r) for r in d_roll]))
            stdscr.addstr(14, 0, str(battle))
            stdscr.addstr(16, 0, "Offense roll {}          ".format(result))
